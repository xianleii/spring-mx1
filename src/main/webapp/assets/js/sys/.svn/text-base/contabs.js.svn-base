...
//	        	console.log("offsetVal:"+offsetVal);
	            ...
	            if((offsetVal + $(tabElement).outerWidth(true)) == offsetVal){
//	            	console.log(11111111111111);
	...
	function scrollTabRight() {
	    ...
	    //实际滚动宽度
	    var scrollVal = 0;
//	    console.log($(".page-tabs-content").width());
	    if ($(".page-tabs-content").width() < visibleWidth) {
	        return false;
	    } else {
	        var tabElement = $(".page-tabs-content .J_menuTab:first");
	        var offsetVal = 0;
	        while ((offsetVal + $(tabElement).outerWidth(true)) <= marginLeftVal) {//找到离当前tab最近的元素
//	        	console.log((offsetVal + $(tabElement).outerWidth(true))+":"+marginLeftVal);
	            offsetVal += $(tabElement).outerWidth(true);
	            tabElement = $(tabElement).next();
	            if((offsetVal + $(tabElement).outerWidth(true)) == offsetVal){
	            	break;
	...
	function menuItem() {
	    ...
	    if (flag) {
	        ...
	        // 添加选项卡对应的iframe
	        var str1 = '<iframe class="J_iframe" name="iframe' + dataIndex + '"  id="iframe' + dataIndex + '" width="100%" height="100%"  src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless  style="overflow: scroll,min-height:500px;"></iframe>';
	        $('.J_mainContent').find('iframe.J_iframe').hide().parents('.J_mainContent').append(str1);

	        // updated by huanghao on 2017/10/12 19:20
            // var outerHeight = $(document.body).height();
            var outerHeight = window.innerHeight;
            var iframeHeight = outerHeight - 47 - 42 -30;
	        $("#iframe"+dataIndex).height(iframeHeight);
	        //显示loading提示
...
//                console.log($(document.getElementById('iframe'+dataIndex).contentWindow.document.body).height())
//                var mainheight = $(document.getElementById('iframe'+dataIndex).contentWindow.document.body).height();
//                $("#iframe"+dataIndex).height(mainheight);
//            });
	    }
	    return false;
	}
	...
	function closeTab() {
	    ...
	    // 不关闭首页 updated by huanghao on 2017/10/12 19:43
        if (closeTabId === "/admin/welcome.do") {
			return;
        }
...
//	    });
	    ...
	    if(isNew || flag){//isNew为true，打开一个新的选项卡； flag为true，选项卡不存在，打开一个新的选项卡。
		        ...
		        // 添加选项卡对应的iframe
		        var str1 = '<iframe class="J_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '"   style="min-height: 1024px;" seamless></iframe>';
